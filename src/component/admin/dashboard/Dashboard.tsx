...
export default function Dashboard() {
  ...
  const semester = useSelector((state: any) => state.semester?.name);
  ...
  useEffect(() => {
    const fetchAPI = async () => {
      if (semester?.name !== '') {
        try {
          const url = BASE_URL + `/semesters/analytics/${semester?.name}`;
  ...
  const handleSemsterResult = async (event: SyntheticEvent) => {
    ...
    if (semester?.status === 'Kết thúc') {
      const respone = await axios({
        method: 'get',
        url: `${BASE_URL}/semesters/get-result/${semester?.name}`,
        headers: authHeader(),
      });
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={3}>
              <Autocomplete
                ...
                disabled={semester?.status !== 'Đang diễn ra'}
            ...
            <Grid item xs={3}>
              <Button
                ...
                disabled={semester?.status === 'Kết thúc'}
              >
                Cập nhật trạng thái
              </Button>
            </Grid>
